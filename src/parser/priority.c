...
/*   Updated: 2025/04/23 17:42:53 by fkuyumcu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_heredoc(line_t *line, int *heredoc_pipe, int *use_heredoc)
{
    line_t	*tmp = line;
    *use_heredoc = 0;
    while (tmp)
    {
        if (tmp->type == HEREDOC_IN)
        {
            ...
            apply_heredoc_pipe(tmp, heredoc_pipe);
            *use_heredoc = 1;
            break;
        }
        tmp = tmp->next;
...
void	execute_pipeline(minishell_t *ms)
{
    ...
    while (i < cmd_count)
    {
        ...
        int use_heredoc = 0;
        handle_heredoc(ms->mini_lines[i], heredoc_pipe, &use_heredoc);
        if (i < cmd_count - 1)
            pipe(pipefd);
        pids[i] = fork();
        if (pids[i] == 0)
        {
            ...
            apply_redirections(ms->mini_lines[i], -1);
