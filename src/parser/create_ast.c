...
/*   Updated: 2025/03/08 13:10:20 by fkuyumcu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ast_node_t	*parse_redirection(token_t tokens[], int *pos,token_type redir_type, minishell_t *ms)
{
	...
	(*pos)++;
	if (tokens[*pos].t_type != WORD)//BUNA BAK
	{
...
ast_node_t	*parse_expression(token_t tokens[], int *pos, minishell_t *ms)
 {
	ast_node_t	*left;
	ast_node_t	*right;
	token_type	type;
	...
	while (tokens[*pos].t_type != TOKEN_END)
	{
		type = tokens[*pos].t_type;
		tokens[*pos].prec = get_precedence(type);
		(*pos)++;
		right = parse_expression(tokens, pos, ms);
		if (!right)
		{
